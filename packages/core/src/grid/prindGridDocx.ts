		...
		sections: [
			{
				children: [
					...
					...grid.flatMap((element) => {
						if (element.questionsGroups.length === 0) {
							return [
								new docx.Paragraph({
									children: [
										new docx.TextRun({
						...
						return [
							...
							...element.questionsGroups.flatMap((element) => {
								const questions = element.questions.flatMap(
									(element, index) => {
								...
								if (
									element.competence &&
									element.competence !== "Sans compétence"
								)
									return [
										new docx.Paragraph({
											children: [
												new docx.TextRun({
													text: `Compétence : ${element.competence}`,
													bold: true,
												}),
											],
											heading: docx.HeadingLevel.HEADING_3,
